...
class _RepoDetailPageState extends ConsumerState<RepoDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<RepoDetailState>(repoDetailNotifierProvider, (pstate, nstate) {
      state.maybeWhen(
        orElse: () {},
        loadSuccess: (repoDetails, _) {
          if (!repoDetails.isFresh) {
    ...
    return WillPopScope(
      onWillPop: () async {
        if (state.isStarredStatusChange) {
          ref
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            state.maybeMap(
              ...
              loadSuccess: (state) {
                return IconButton(
                  onPressed: !state.repoDetails.isFresh
                      ? null
                      : () => ref
                          .read(repoDetailNotifierProvider.notifier)
                          .switchStarredStatus(state.repoDetails.entity!),
                  icon: Icon(
                    !state.repoDetails.isFresh
                        ? MdiIcons.starRemoveOutline
                        : state.repoDetails.entity?.starred == true
                            ? Icons.star
                            : Icons.star_outline,
                  ),
                );
        ...
        body: state.map(
          ...
          loadSuccess: (state) {
            if (state.repoDetails.entity == null) {
              return const NoResultDisplay(
                message:
                    'There are all information when could possibly find right now',
              );
            } else {
              return WebView(
                ...
                initialUrl: Uri.dataFromString(
                  ...
                  <body>${state.repoDetails.entity?.html}</body>
                  <style>$css</style>
                  <html>
                  ''',
                  mimeType: 'text/html',
